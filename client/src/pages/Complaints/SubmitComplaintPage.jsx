...
// Same default center used by the Complaints Map page (Itahari, Koshi Province)
// so a freshly-opened picker starts somewhere relevant to the platform.
const DEFAULT_CENTER = [26.665, 87.275];
...
// instead so the pin actually renders instead of a broken image icon.
...
// Listens for map clicks and reports the picked lat/lng up to the parent.
...
// MapContainer only honours `center`/`zoom` on first mount, so when
// "Use my location" sets coordinates programmatically (not via a click on
// the map itself) we need to explicitly pan the already-mounted map there.
...
export default function SubmitComplaintPage() {
  ...
  const [form, setForm] = useState({
    ...
    priority: "Medium", description: "", latitude: null, longitude: null,
  });
  ...
  const useMyLocation = () => {
    if (!navigator.geolocation) {
      setLocateError("Your browser doesn't support location detection. Please pin it on the map instead.");
      return;
    }
    setLocating(true);
    setLocateError("");
    navigator.geolocation.getCurrentPosition(
      ...
      () => {
        setLocateError("Couldn't get your current location. Please pin it on the map instead.");
        setLocating(false);
      },
  ...
  const resetForm = () => {
    setForm({ category: "", subCategory: "", title: "", address: "", wardNumber: "", priority: "Medium", description: "", latitude: null, longitude: null });
    setLocateError("");
    setFiles([]);
    setErrors({});
    setCreated(null);
  ...
  return (
    <DashboardLayout>
      <div style={{
        ...
        <img
          src="https://images.unsplash.com/photo-1537511446984-935f663eb1f4?auto=format&fit=crop&w=1200&q=80"
      ...
      <form onSubmit={handleSubmit} noValidate style={{ maxWidth: 760 }}>
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          {/* Pin the exact spot on the map so this complaint shows up on the
              Complaints Map page — without lat/lng the complaint is invisible there. */}
          <div style={{ marginTop: 10 }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
              ...
              <button
                ...
                disabled={locating}
                style={{
                  fontSize: 11.5, fontWeight: 600, color: "var(--primary)", background: "none",
                  border: "1px solid var(--primary)", borderRadius: 6, padding: "4px 10px",
            ...
            <div style={{ borderRadius: 10, overflow: "hidden", border: "1px solid var(--border)", height: 220 }}>
              <MapContainer
                center={form.latitude != null ? [form.latitude, form.longitude] : DEFAULT_CENTER}
                zoom={form.latitude != null ? 15 : DEFAULT_ZOOM}
              ...
              >
                ...
                />
                <LocationClickHandler onPick={pickLocation} />
                <RecenterOnChange lat={form.latitude} lng={form.longitude} />
                {form.latitude != null && (
                  <Marker position={[form.latitude, form.longitude]} icon={markerIcon} />
                )}
              </MapContainer>
            </div>
            {locateError && <div style={{ color: "var(--accent)", fontSize: 11.5, marginTop: 4 }}>{locateError}</div>}
          </div>
        </div>
